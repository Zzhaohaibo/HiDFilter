...
from hidfilter.runtime.self_only import self_only_forward
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen Phase 1 Self-only CUDA sanity")
    parser.add_argument(
        "--config",
        type=Path,
        default=REPOSITORY_ROOT / "configs" / "phase1_self_pems08.json",
    )
...
def main() -> None:
    ...
    model = SelfOnlyHiDFilter(NUM_NODES).to(device=device, dtype=torch.float32)
    ...
    if epochs >= 2 and not loss_decreased:
        raise RuntimeError("Self-only training loss did not decrease")
