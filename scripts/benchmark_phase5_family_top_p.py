...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Local Phase 5 Family Top-p performance diagnostic"
    )
...
def main() -> None:
    ...
    for _ in range(args.warmup):
        _full_step(model, optimizer, history, target, family_top_p_enabled=False)
        _full_step(model, optimizer, history, target, family_top_p_enabled=True)
    _synchronize(device)
    if device.type == "cuda":
        torch.cuda.reset_peak_memory_stats(device)

    total_step_off_ms, _ = _measure(
        lambda: _full_step(
            model, optimizer, history, target, family_top_p_enabled=False
        ),
        device=device,
        iterations=args.iterations,
    )
    total_step_on_ms, _ = _measure(
        lambda: _full_step(
            model, optimizer, history, target, family_top_p_enabled=True
        ),
        device=device,
        iterations=args.iterations,
    )
    with torch.no_grad():
        total_forward_off_ms, _ = _measure(
            lambda: model(history, family_top_p_enabled=False),
            device=device,
            iterations=args.iterations,
        )
        total_forward_on_ms, _ = _measure(
            lambda: model(history, family_top_p_enabled=True),
            device=device,
            iterations=args.iterations,
        )
    forward_backward_off_ms, _ = _measure(
    ...
    forward_backward_on_ms, _ = _measure(
        lambda: _forward_backward(
            model, optimizer, history, target, family_top_p_enabled=True
        ),
        device=device,
    ...
    with torch.no_grad():
        ...
        dense_router_ms, probability = _measure(
            lambda: model.compute_router_probability(context, evidence),
            device=device,
            iterations=args.iterations,
        )
        family_top_p_ms, family_routing = _measure(
            lambda: family_top_p(
                probability,
        ...
        _, family_weight = family_routing
        messages = tuple(
            torch.randn(
                BATCH_SIZE,
                NUM_NODES,
        ...
        dense_fusion_ms, _ = _measure(
            lambda: dense_router_fusion(*messages, probability),
            device=device,
            iterations=args.iterations,
        )
        top_p_fusion_ms, _ = _measure(
            lambda: dense_router_fusion(*messages, family_weight),
            device=device,
            iterations=args.iterations,
        )
    ...
    print(f"dense_router_ms={dense_router_ms:.3f}")
    print(f"family_top_p_ms={family_top_p_ms:.3f}")
    print(f"dense_fusion_ms={dense_fusion_ms:.3f}")
    print(f"top_p_fusion_ms={top_p_fusion_ms:.3f}")
    print(f"total_forward_off_ms={total_forward_off_ms:.3f}")
    print(f"total_forward_on_ms={total_forward_on_ms:.3f}")
    print(f"forward_backward_off_ms={forward_backward_off_ms:.3f}")
    print(f"forward_backward_on_ms={forward_backward_on_ms:.3f}")
    print(f"optimizer_ms={optimizer_ms:.3f}")
    print(f"total_step_off_ms={total_step_off_ms:.3f}")
    print(f"total_step_on_ms={total_step_on_ms:.3f}")
    if device.type == "cuda":
...
def _forward_backward(
    ...
    *,
    ...
    prediction = model(history, family_top_p_enabled=family_top_p_enabled)
...
def _optimizer_step(
    model: HiDFilter, optimizer: torch.optim.Optimizer
) -> torch.Tensor:
