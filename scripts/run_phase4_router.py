...
from hidfilter.runtime.router import collect_router_probability_summary, router_forward
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen Phase 4 dense Router CUDA sanity")
    parser.add_argument(
        "--config",
        type=Path,
        default=REPOSITORY_ROOT / "configs" / "phase4_router_pems08.json",
    )
...
def main() -> None:
    ...
    try:
        for epoch_index in range(epochs):
            ...
            metadata = {
                ...
                "model": "HiDFilter Self+Physical+Semantic+DenseRouter",
    ...
    router_summary = collect_router_probability_summary(model, val_loader, scaler, device)
    ...
    family_order = router_summary.family_order
    report = {
        "phase": 4,
        "model": "HiDFilter Self+Physical+Semantic+DenseRouter",
        ...
        "router": {
            ...
            "top_p_enabled": False,
            "probability_summary": {
                "overall_mean": dict(zip(family_order, router_summary.overall_mean)),
                "minimum": dict(zip(family_order, router_summary.minimum)),
                "maximum": dict(zip(family_order, router_summary.maximum)),
                "positions": router_summary.positions,
                "seconds": router_summary.seconds,
            },
        },
        "config": {
            "batch_size": int(config["batch_size"]),
            ...
            "family_fusion": "availability-aware dense Router",
        },
        "timer_boundaries": {
            ...
            "router_probability_summary": "separate post-reload diagnostic; one final device transfer",
        },
    ...
    if epochs >= 2 and not loss_decreased:
        raise RuntimeError("dense-Router training loss did not decrease")
