...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen Phase 2 Physical CUDA sanity")
    parser.add_argument(
        "--config",
        type=Path,
        default=REPOSITORY_ROOT / "configs" / "phase2_physical_pems08.json",
    )
    parser.add_argument("--dataset-dir", type=Path)
    parser.add_argument("--adjacency-path", type=Path)
    parser.add_argument("--candidate-cache-path", type=Path)
...
def main() -> None:
    ...
    candidate_cache_path = _repository_path(
        args.candidate_cache_path or Path(config["candidate_cache_path"])
    )
    ...
    candidate_summary = summarize_physical_candidates(preparation.artifact)
    ...
    num_workers = (
        ...
        else select_worker_count(worker_results)
    )
    ...
    model = HiDFilter(
        NUM_NODES,
        physical_candidates=preparation.artifact.candidates,
    ...
    try:
        for epoch_index in range(epochs):
            ...
            metadata = {
                ...
                "model": "HiDFilter Self+Physical",
                "physical_candidate_fingerprint": preparation.artifact.fingerprint,
            }
    ...
    report = {
        ...
        "graph": {
            ...
            **adjacency_evidence,
            ...
            "candidate_build_seconds": preparation.seconds,
            **candidate_summary,
        },
        "config": {
            "batch_size": int(config["batch_size"]),
            ...
            "traffic_only": True,
            "edge_top_p_rho": 0.8,
            "dependency_spaces": ["Self", "Physical"],
        ...
        "summary": {
            ...
            "physical_candidate_build_seconds": preparation.seconds,
        },
    }
    ...
    if epochs >= 2 and not loss_decreased:
        raise RuntimeError("Self+Physical training loss did not decrease")
