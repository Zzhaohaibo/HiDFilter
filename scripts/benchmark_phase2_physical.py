...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Local Phase 2 Physical performance diagnostic")
...
def main() -> None:
    ...
    adjacency = np.ones((170, 170), dtype=np.float64)
    np.fill_diagonal(adjacency, 0.0)
    build_started = time.perf_counter()
    artifact = build_physical_candidates(
        adjacency,
        PhysicalGraphContract("undirected", "affinity", None),
        kp=8,
    )
    candidate_build_seconds = time.perf_counter() - build_started
    model = HiDFilter(170, physical_candidates=artifact.candidates).to(
    ...
    with torch.no_grad():
        ...
        key_projection_ms, physical_key_global = _measure(
            lambda: model.wk(
                fine_tokens + family_identity.view(1, 1, 1, HIDDEN_DIM)
            ),
            device=device,
            iterations=args.iterations,
        )
        gather_ms, gathered = _measure(
            lambda: (
                gather_candidates(physical_key_global, model.physical_flat_index),
                gather_candidates(value_global, model.physical_flat_index),
            ),
            device=device,
            iterations=args.iterations,
        )
        ...
        fine_ms, dense_probability = _measure(
            lambda: _physical_dense_probability(model, query, candidate_key),
            device=device,
            iterations=args.iterations,
        )
        valid = model.physical_valid.view(1, 170, 1, 96)
    ...
    print("shape=B64,N170,L12,H12,D64,Cself12,Cphysical96")
    print(f"candidate_build_seconds={candidate_build_seconds:.6f}")
    ...
    print(f"physical_key_projection_ms={key_projection_ms:.3f}")
    print(f"physical_gather_ms={gather_ms:.3f}")
    print(f"physical_fine_score_softmax_ms={fine_ms:.3f}")
    print(f"physical_top_p_ms={top_p_ms:.3f}")
    print(f"physical_message_ms={message_ms:.3f}")
    if device.type == "cuda":
...
def _shared_fine_inputs(
    ...
    family_identity = model.fine_family_projection(
        model.fine_family_embedding.weight[PHYSICAL_FAMILY_ID]
    )
    return query, fine_tokens, value_global, family_identity


def _physical_dense_probability(
    model: HiDFilter,
    query: torch.Tensor,
    ...
    score = score + (
        model.alpha * torch.log(model.physical_prior.clamp_min(1.0e-6))
    ).view(1, 170, 1, 96)
    return safe_masked_softmax(score, model.physical_valid.view(1, 170, 1, 96))
